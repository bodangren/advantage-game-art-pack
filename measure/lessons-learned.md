...
- (2026-05-02, directional_character_sheets_20260502) Adding a new compiler family requires: dataclass program type, layout mode constant, _load_*_program parser, _compile_* function, and wire into CompilerRegistry.definitions. When adding new family to FAMILY_NAMES, update corpus_manifest.json taxonomy.families and any test manifests that hardcode the family list.
- (2026-05-02, directional_character_sheets_20260502) Style pack directional_variants uses nested dict: top-level keys are direction strings (N/S/E/W), nested dict maps part_name to variant_id. Empty dict {} is valid (no directional overrides).
- (2026-05-02, directional_character_sheets_20260502) DirectionalSheetProgram.render_spec is optional (nullable) since directional sheets may not need a full SpriteSpec. Check for None before using.
- (2026-05-02, directional_character_sheets_20260502) `asf compile` already handles directional_sheet via generic dispatch; no new subcommand needed. Example programs at programs/directional_sheet/ demonstrate usage.
