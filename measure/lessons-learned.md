...
- (2026-05-09, critic_reference_calibration) FAMILY_NAMES has 14 families but canon.py and compilers.py use different naming (e.g., prop_or_fx_sheet vs prop_sheet/fx_sheet, parallax_layer_set vs parallax_layer). Presentation surfaces use surface_family field not family field. Some primitives lack source.png files, causing assembly failures for background_scene, ui_sheet, presentation_surface.
- (2026-05-09, critic_reference_calibration) ReferenceAssetLoader provides a clean abstraction for loading reference PNGs from reference/demo_assets/. Family-to-layout-type mapping is needed since FAMILY_NAMES doesn't directly map to LAYOUT_TYPES.
